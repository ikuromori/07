...
// データベースと接続
try {
  $pdo = new PDO('mysql:dbname=ikuromori_a_db;charset=utf8;host=mysql57.ikuromori.sakura.ne.jp', '********', '*******');
...
// データベースに挿入
$stmt = $pdo->prepare("INSERT INTO b_table(id, u_name, u_password
...
// // データベースからIDを引き出す
$stmt2 = $pdo->prepare("SELECT * FROM b_table WHERE u_name IN('$name')");
...
// // データベースからIDを引き出す
$stmt = $pdo->prepare("SELECT * FROM b_table WHERE u_name IN('$name')");
$status = $stmt->execute();
